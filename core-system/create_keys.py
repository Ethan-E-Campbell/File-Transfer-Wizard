...
def create_pgp_key_pair(key_type, key_length, name_real, name_comment, name_email):
    ...
    ascii_armored_public_keys = gpg.export_keys(key_id, passphrase=password) # same as gpg.export_keys(keyids, False)
    ascii_armored_private_keys = gpg.export_keys(key_id, True, passphrase=password) # True => private keys
