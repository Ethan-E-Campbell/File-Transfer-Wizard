...
def encrypt_file(key_id, file_byte_data,
                 file_name, output_file_name_with_ext,
                 output_path=None):
    try:
        print("Starting file encryption process....")
        gpg = create_gpg()
        if (
            output_file_name_with_ext[-3:] != ".pgp"
                and output_file_name_with_ext.find(".") == -1):
        ...
        result = gpg.encrypt(file_byte_data,
                             recipients=key_id,
